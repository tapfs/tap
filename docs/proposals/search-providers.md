...
`tap search "cats"` → `Global`. `tap search github "cats"` → `Connector`.
`tap search github/issues "cats"` → `Collection`.
...
Providers come from `~/.tapfs/search.toml` (or `--search-config`):

```toml
[[providers]]
name   = "qmd"
kind   = "process"           # spawn a child process speaking JSON-RPC
command = "qmd"
args    = ["mcp"]
...
The `process` and `http` provider kinds are scaffolded as `Err(NotSupported)`
in this PR so the seam is real but the wire protocol is left for a follow-up.
...
tap search github "cats"                # all eligible + upstream
tap search github/issues "cats"         # narrower
...
- `process` and `http` provider kinds defined in config but not wired
