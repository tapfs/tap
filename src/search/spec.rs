...
//!
//! Loaded from `~/.tapfs/search.yaml`. Three `kind`s are recognized:
//! `builtin` (in-tree Rust impl), `process` (child process speaking JSON-RPC),
//! and `http` (centralized HTTP backend).
//!
...
pub struct ProviderSpec {
    ...
    /// Restrict the provider's default `scopes()`. If omitted, the provider's
    /// own declaration is used. Config cannot expand a provider beyond the
    /// scope kinds it declares.
    #[serde(default)]
