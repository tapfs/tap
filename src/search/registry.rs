...
impl SearchRegistry {
    ...
    fn eligible(&self, scope: &SearchScope) -> Vec<Arc<dyn SearchProvider>> {
        ...
        let mut out: Vec<Arc<dyn SearchProvider>> = self
            ...
            .filter(|p| !excluded.iter().any(|e| e == p.key()))
            .map(|p| p.value().clone())
            .collect();
...
mod tests {
    ...
    async fn fans_out_to_all_eligible_providers() {
        ...
        let r = reg.query(SearchScope::Global, Query::new("x")).await.unwrap();
    ...
    async fn slow_provider_times_out_without_blocking_others() {
        ...
        assert_eq!(r.hits.len(), 1);
        assert!(r.warnings.iter().any(|w| w.contains("slow") && w.contains("deadline")));
    }
